    ...
    public function alertCourier($id){
        ...
        foreach($order->items AS $item){
            if($item->shipping_method == "courier"){
                ...
                $vnd_det = $product->courierDetails;
                ...
                $parcels = [
                    ...
                    "parcel_description" => $product->item_description,
                    "alternative_tracking_reference" => "AB-ORD".str_pad($order_id, 4, '0', STR_PAD_LEFT),
                ];
                ...
                $vendor_detail = [
                    "name" => $product->user->name.' '.$product->user->surname,
                    "email" => $product->user->email,
                    "mobile_number" => $product->user->mobile_number
                ];

                $buyer_details = [
                    "name" => $item->user->name.' '.$product->user->surname,
    ...
    public function init(){
        ...
        foreach($orders AS $order){
            ...
            if($res['status'] == "successful"){
                ...
                $paid_amount = number_format($res['amount']/100,2);

                if($res['payment_method'] == "card"){
                    $stn = Setting::where('name', 'card_fee')->first();
                    $card_fee = $stn->value;
                    $to_pay = $order->cart_total + (($card_fee/100) * $order->cart_total);
                }
                else{
                    $to_pay = $order->cart_total;
                }
                ...
                $order->amount_paid = $res['amount']/100;
                $order->status = 'COMPLETE';
                $order->save();

                $this->alertCourier($order->id);
                ...
                foreach($order->items AS $item){
                    ...
                    if($fee){
                        if($fee < $min_fee_val && $product->service_fee_payer != "50-50"){
                            $fee = $min_fee_val;
                        }
                        Transaction::create([
                            'name' => 'service_fee',
